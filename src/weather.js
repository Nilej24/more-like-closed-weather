async function fetchWeatherOf(city, tempUnit) {
  ...
  const weatherResponse = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${key}&units=${tempUnit}`);
